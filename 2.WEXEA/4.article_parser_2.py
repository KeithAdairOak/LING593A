...
#            if len(token) > 1 and (token in persons or i == len(tokens) - 1):
...
#                        redirects[mention] in mostly_upper_articles or not redirects[mention] in mostly_lower_articles):
                        redirects[mention] in mostly_upper_articles):
...
#                        entity = candidates[0][0]
                    candidates_tmp = aliases.get(mention)
                    try:
                        if candidates_tmp:
                            candidates = candidates_tmp["list"]   
                            entity = candidates[0][0]
                            if len(candidates) == 1:
                                annotation_type = 'UPPER_ENTITY'
                            else:
...
def annotate_entities(sentence,article_aliases,article_aliases_list,
#sentence_starters
    start_of_sentence,
    entities,all_found_aliases,file_entity,all_entities,unknown_persons):
    found_entities = []
    end_of_sentence = ''
    if len(sentence) == 0:
        return sentence
...
#    doc = nlp(sentence)
#    print([t.text for t in doc.ents])
        #if token.pos_ in ['NOUN', 'PROPN']: # NOUNが名詞、PROPNが固有名詞
#    [print(t.text ) for t in doc.noun_chunks]
#    [print(t.text, t.tag_) for t in doc]
#    [print(t.text) for t in doc.ents]
#    annotation_spacy = {(t.start_char,len(t.text)):t.label_.upper() for t in doc.ents}
#    [print(t) for t in doc.ents]
#    [found_entities.append((t.start_char,len(t.text))) for t in doc.ents]
#    previous =''
#    first_word_index = -1
...
#        print("F",first_mention)
#        print("B",between)
#        print("S",second_mention)
...
#            if 'UNKNOWN' == annotation_type:
#                if (start,length) in annotation_spacy:
#                    if annotation_spacy[(start,length)] == "DATE":
...
#        doc = nlp(line)
#        sent_tokenize_list = [str(l) for l in doc]
#        sent_tokenize_list = [str(l) for l in doc.noun_chunks]
...
#            if i > 0 and len(sent_tokenize_list[i]) > 0 and sent_tokenize_list[i].lstrip()[0].isalpha() and sent_tokenize_list[i].lstrip()[0].islower():
            if i > 0 and len(sent_tokenize_list[i]) > 0 :
...
#    other_entity_content = content[pos_other+14:]
    pos_actual = content.find(ACTUAL)
    pos_other = content.find(OTHER)
    actual_entity_content = content[pos_actual+len(ACTUAL):pos_other]
    other_entity_content = content[pos_other+len(OTHER):]
...
#        [print(t) for t in sent_tokenize_list]
...
#                idx = 0
                ...
                #####
                while True:
                    start = content.find('{{TEMPLATE|読み仮名')
...
#                        end += idx     
                        ...
                        if end > -1:
                            ...
                            template = content[start+11:end].split("|")[1:]
                            opening = False
                            tmps = ""
                            tmpl = []
                            for c in template:
                            ...
                            template = tmpl
                            if len(template) < 3:
                                for _ in range(3-len(template)):
                                    template.append("")
                            content = content[:start] + template[0] + "（" + template[1] + "、" + template[2] + "）" + content[end+2:]
                        else:
                            break
                    else:
                        break
                while True:
                    start = content.find('{{TEMPLATE|読み')
                    if start < 0:
                        start = content.find('{{TEMPLATE|Ruby')
#                    end = content[start:].find('}}')
                    if start > -1:# and end > -1 and end > start:
                        end = content[start:].find('}}')
#                        start += idx
#                        end += idx     
                        if end > -1:
                            ...
                            template = tmpl
                            if len(template) < 2:
                                for _ in range(2-len(template)):
                                    template.append("")
                        
                            content = content[:start] + template[0] + "（" + template[1] + "）" + content[end+2:]
                        else:
                            break                
                    else:
                        break                
                while True:
                    start = content.find('{{TEMPLATE|仮リンク')
#                    end = content[start:].find('}}')
                    if start > -1:# and end > -1 and end > start:
                 #       start += idx
                 #       end += idx
                        ...
                        if end > -1:
                            ...
                            tmp = content[start+11:end].split("|")
                            template = [t for t in tmp if t.startswith("label=")] 
                                
                            if template:
                                template = template[0].split("label=")[1]
                            else:
                                template = content[start+11:end].split("|")[1]
                            content = content[:start] + template + content[end+2:]
                        else:
                            break                
                    else:
                        break                
                ...
                while True:
                    start = content.lower().find('{{template|lang')
                    if start < 0:
                        start = max([content.lower().find('{{template|'+ lang + '|') for lang in LANG_CODES])
                    end = content[start:].find('}}')
                    if start > -1 and end > -1:# and end > start:
#                        start += idx
                        end += start     
                        ...
                        for c in template:
                            if "[[" in c:
                                ...
                                tmpl.append(c)
                        if tmps and tmps not in tmpl:
                            tmpl.append(tmps)
                        template = tmpl
#                        print(file_entity,template)
                        if len(template) > 0:
                            content = content[:start] + template[-1] + content[end+2:]
                        else:
...
#                        print(idx,start,previous_part)
                        previous_part = annotate_entities(previous_part, article_aliases, article_aliases_list, #sentence_starters,
                            start_of_sentence,entities,all_found_aliases,file_entity,all_entities,unknown_persons)
...
#                            if entity == alias or entity in alias:
                            if entity == alias:

                                alias = annotate_entities(alias, article_aliases, article_aliases_list,
                                                              #sentence_starters, 
                                                              start_of_sentence, entities,
                                                              all_found_aliases, file_entity,all_entities,unknown_persons)

                                first_part = content[:idx] + previous_part
#                            elif alias in entity:

#                                first_part = content[:idx] + previous_part + entity
...
#    persons = set(json.load(open('data\\persons.json',encoding="utf8")))
#    sentence_starters = set(json.load(open('data\\frequent_sentence_starters.json',encoding="utf8")))

    aliases_reversed = json.load(open(dictionarypath + 'aliases_reversed_sorted_pruned_upper.json',encoding="utf8"))
    aliases = json.load(open(dictionarypath + 'aliases_sorted_pruned_upper.json',encoding="utf8"))
    redirects_reversed = json.load(open(dictionarypath + 'redirects_reversed_upper.json',encoding="utf8"))
    mostly_upper_articles = set(json.load(open(dictionarypath + 'mostly_upper.json',encoding="utf8")))
...
#    redirects = json.load(open(dictionarypath + 'redirects.json',encoding="utf8"))
    redirects = json.load(open(dictionarypath + 'redirects_upper.json',encoding="utf8"))
    all_articles = set(json.load(open(dictionarypath + 'articles.json',encoding="utf8")))
    disambiguations = json.load(open(dictionarypath + 'disambiguations_other.json',encoding="utf8"))
    human_disambiguations = json.load(open(dictionarypath + 'disambiguations_human.json',encoding="utf8"))
    geo_disambiguations = json.load(open(dictionarypath + 'disambiguations_geo.json',encoding="utf8"))
    given_names = json.load(open(dictionarypath + 'given_names_dict.json',encoding="utf8"))
    filename2title = json.load(open(dictionarypath + 'filename2title.json',encoding="utf8"))
    links = json.load(open(dictionarypath + 'links.json',encoding="utf8"))
    title2Id = json.load(open(dictionarypath + 'title2Id.json',encoding="utf8"))
    most_popular = json.load(open(dictionarypath + 'most_popular_entities_10000.json',encoding="utf8"))
    ...
    diff_acc = 0.0
    c = 0
    progress = tqdm.tqdm(total = len(article_directories))
