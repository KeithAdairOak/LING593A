...
RE_REMOVE_SECTIONS = re.compile(r'==[\s|　]*関連項目|==[\s|　]*関連書籍|==[\s|　]*注釈|==[\s|　]*脚注|==[\s|　]*出典|==[\s|　]*参考',
                                re.DOTALL | re.UNICODE)
#RE_FILES = re.compile(r'\[\[(Image|File)(.*)\n?', re.UNICODE)
RE_FILES = re.compile(r'\[\[(Image|File|ファイル:)(.*)\n?', re.UNICODE)
...
RE_DISAMBIGUATIONS = '{{set index article}}|{{SIA}}|{{Aimai}}|{{曖昧さ回避\||\|曖昧さ回避}}|{{曖昧さ回避}}|{{disambiguation\||\|disambiguation}}|{{disambiguation}}|{{disamb}}|{{disambig}}|{{disamb\||\|disamb}}|{{disambig\||\|disambig}}|{{dab\||\|dab}}|{{dab}}|{{disambiguation cleanup}}'
RE_HUMAN_DISAMBIGUATIONS = '{{hndis\||\|hndis}}|{{hndis}}|{{人名の曖昧さ回避}}|{{人名の曖昧さ回避\||\|人名の曖昧さ回避}}|{{human name disambiguation}}|{{human name disambiguation\||\|human name disambiguation}}'
RE_GEO_DISAMBIGUATIONS = '{{place name disambiguation}}|{{geodis}}|{{geodis\||\|geodis}}|{{同名の地名}}|{{地名の曖昧さ回避}}'
RE_NUMBER_DISAMBIGUATIONS = '{{number disambiguation\||\|number disambiguation}}|{{numdab\||\|numdab}}|{{numberdis\||\|numberdis}}|{{numberdis}}|{{numdab}}|{{number disambiguation}}'
RE_STUB = 'stub}}'
...
GIVEN_NAMES = '\{\{given name\}\}', '\[\[Category:.*?人名\]\]','\[\[Category:.*?性名\]\]','\[\[Category:人名の短縮形\]\]','\[\[Category:Given names\]\]', '\[\[Category:Masculine given names\]\]', '\[\[Category:Feminine given names\]\]'
SURNAMES = '\{\{surname\}\}', '\[\[Category:.*?姓\]\]', '\[\[Category:Surnames\]\]'
...
TEMPLATES_TO_KEEP = ['cvt','convert','quote','lang','wikt-lang','as of',"読み","ruby","仮リンク",
'aa|',
'ab|',
'ae|',
'af|',
...
class WikiHandler(xml.sax.ContentHandler):
    ...
    def processArticle(self):
        ...
        or self.title.startswith('ファイル:')\
        or self.title.startswith('モジュール:'):
            redirect = self.title
            pass
            #do nothing
...
#            filename = self.outputpath + 'disambiguations_human\\' + self.title.replace(" ", '_').replace('\','_') + '.txt'
            filename = self.outputpath + 'disambiguations_human\\' + conv_filename(self.title) + '.txt'
...
#            filename = self.outputpath + 'disambiguations_geo\\' + self.title.replace(" ", '_').replace('\\','_') + '.txt'
            filename = self.outputpath + 'disambiguations_geo\\' +  conv_filename(self.title) + '.txt'
        #    if os.path.exists(filename):
        #        pass
        #    else:
            with open(filename, 'w',encoding="utf8") as f:
...
#            filename = self.outputpath + 'disambiguations_number\\' + self.title.replace(" ", '_').replace('\\','_') + '.txt'
            filename = self.outputpath + 'disambiguations_number\\' + conv_filename(self.title) + '.txt'
...
#        elif any(re.findall(RE_DISAMBIGUATIONS, text.lower())) or '(disambiguation)' in self.title:
        elif any(re.findall(RE_DISAMBIGUATIONS, text.lower())) or ('曖昧' in self.title and '回避' in self.title):
            self.disambiguations[self.title] = []
#            filename = self.outputpath + 'disambiguations_other\\' + self.title.replace(" ", '_').replace('\\', '_') + '.txt'
            filename = self.outputpath + 'disambiguations_other\\' + conv_filename(self.title) + '.txt'
         #   if os.path.exists(filename):
         #       pass
         #   else:
...
#            if any(given_name in text for given_name in GIVEN_NAMES):
            if any(re.search(given_name,text) for given_name in GIVEN_NAMES):
                    self.given_names.append(self.title)
#            if '{{surname}}' in text or '[[Category:Surnames]]' in text:
            if any(re.search(surname,text) for surname in SURNAMES):
                    self.surnames.append(self.title)
#            file_directory = re.sub('[^0-9a-z]+', '_', self.title.lower())
            fname = conv_filename(self.title)
#            file_directory = self.title.lower().replace(" ", '_').replace('/', '_')
            if len(self.title) == 1:
                file_directory = "_"
            elif fname[0] == ".":
                file_directory = "dot"
            else:
                file_directory = fname[0]
...
#            file_directory = self.outputpath + 'original_articles/' + file_directory + '/'
            ...
            filename = file_directory + fname + '.txt'

            self.filename2title[filename] = fname
            self.articles.append(fname)
...
#            if not self.title.lower().startswith('list of'):
            if not any(ignore in self.title for ignore in IGNORED_NAMESPACES_JA):
...
#        if entity.lower().startswith('list of'):
        if any(ignore in entity for ignore in IGNORED_NAMESPACES_JA):
            return
...
#        first_line_break = text.find('\n')
        ####
        first_line_maru = text.find('。')
#        first_line_nline = text.find('\n')
#        if first_line_maru < first_line_nline \
#        and first_line_maru > -1:
        first_line_break = first_line_maru
#        else:
#            first_line_break = first_line_nline
        ####
        if first_line_break == -1:
            first_line_break = len(text)
        
        sliced = text[:first_line_break]
        while True:
            e = sliced.rfind("'''")
            if e > -1:
                sliced = sliced[:e]
...
#            m = re.search(RE_TEMPLATE_1,text[])
            m = re.search(RE_TEMPLATE_1,text[idx:])
            if m == None:
                break
#            start = m.span()[0]
#            end = m.span()[1]
...
#                        if not entity.lower().startswith('file:') and not entity.lower().startswith('image:') and not entity.lower().startswith('list of'):
                        if not entity.lower().startswith('file:') \
                        and not entity.lower().startswith('image:') \
                        and not entity.lower().startswith('ファイル：') \
                        and not any(ignore in entity for ignore in IGNORED_NAMESPACES_JA):
...
#    os.mkdir(articlepath, mode)
    ...
    if os.path.exists(outputpath + 'disambiguations_human\\'):
        pass
    else:
        os.mkdir(outputpath + 'disambiguations_human\\', mode)

    if os.path.exists(outputpath + 'disambiguations_number\\'):
        pass
    else:
        os.mkdir(outputpath + 'disambiguations_number\\', mode)

    if os.path.exists(outputpath + 'disambiguations_geo\\'):
        pass
    else:
        os.mkdir(outputpath + 'disambiguations_geo\\', mode)

    if os.path.exists(outputpath + 'disambiguations_other\\'):
        pass
    else:
...
#    os.mkdir(outputpath + 'disambiguations_other\\', mode)
    ...
    aliases = {}
    redirects = json.load(open(dictionarypath + 'redirects.json',encoding="utf8"))
    disambiguations = {}
    human_disambiguations = {}
    geo_disambiguations = {}
    number_disambiguations = {}
    title2Id = json.load(open(dictionarypath + 'title2Id.json',encoding="utf8"))
    links = {}
    link_probs = {}
    filename2title = {}
    given_names = []
    ...
    diff = current_milli_time() - Handler.start
    print('Articles processed: ' + str(Handler.counter_all) + ', avg t: ' + str(diff / Handler.n), end='\r')
    print('\ndone')
    ...
    with open(dictionarypath + 'aliases.json','w',encoding="utf8") as f:
        json.dump(aliases,f,indent=4,ensure_ascii=False)
    with open(dictionarypath + 'disambiguations_other.json', 'w',encoding="utf8") as f:
        json.dump(disambiguations,f,indent=4,ensure_ascii=False)
    with open(dictionarypath + 'disambiguations_human.json','w',encoding="utf8") as f:
        json.dump(human_disambiguations,f,indent=4,ensure_ascii=False)
    with open(dictionarypath + 'disambiguations_geo.json','w',encoding="utf8") as f:
        json.dump(geo_disambiguations,f,indent=4,ensure_ascii=False)
    with open(dictionarypath + 'disambiguations_number.json','w',encoding="utf8") as f:
        json.dump(number_disambiguations,f,indent=4,ensure_ascii=False)
    with open(dictionarypath + 'given_names.json', 'w',encoding="utf8") as f:
        json.dump(given_names, f,indent=4,ensure_ascii=False)
    with open(dictionarypath + 'surnames.json', 'w',encoding="utf8") as f:
        json.dump(surnames, f,indent=4,ensure_ascii=False)
    with open(dictionarypath + 'filename2title.json', 'w',encoding="utf8") as f:
        json.dump(filename2title, f,indent=4,ensure_ascii=False)
    with open(dictionarypath + 'title2filename.json', 'w',encoding="utf8") as f:
        json.dump(title2filename, f,indent=4,ensure_ascii=False)
    with open(dictionarypath + 'articles.json', 'w',encoding="utf8") as f:
        json.dump(articles, f,indent=4,ensure_ascii=False)
    with open(dictionarypath + 'links.json', 'w',encoding="utf8") as f:
        json.dump(links, f,indent=4,ensure_ascii=False)
    with open(dictionarypath + 'stubs.json', 'w',encoding="utf8") as f:
        json.dump(links, f,indent=4,ensure_ascii=False)
